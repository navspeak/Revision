...
## RagController — quick topK testing without Copilot

```bash
./gradlew bootRun   # normal profile — NOT --spring.profiles.active=mcp,
                     # since that disables the web server entirely
```

```bash
curl -s -X POST "http://localhost:8080/rag/search" \
  -H "Content-Type: application/json" \
  -d '{"query": "return window for damaged items", "topK": 3}' | jq
```
...
```json
[
  { "text": "## 2. Returns and Refunds\n\n...", "score": 0.83 },
  { "text": "## 1. Order Cancellation\n\n...", "score": 0.41 },
  { "text": "## 5. Escalation Procedure\n\n...", "score": 0.29 }
]
```

Good things to try:
- `topK: 1` vs `topK: 7` (the whole doc's section count) on the same
  query — watch the score drop-off to get a feel for where irrelevant
  passages start creeping in, useful for picking a sane default topK.
- The same query right after editing `oe-support-policy.md` + rebuilding
  — confirms ingestion is picking up your latest content (see the "100
  days" edit walkthrough from earlier).
